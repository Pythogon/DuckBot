...
class Starboard(commands.Cog):
    ...
    async def on_raw_reaction_add(self, p: discord.RawReactionActionEvent):
        ...
        ### Checks ###
        if p.emoji.name != "⭐": return # Only looking for :star:
        if message.author.bot: return # Bots cannot be added to starboard
        if discord.utils.get(message.reactions, me = True, emoji = "🌟"): return # :star2: is indicator of prior starboard, prevents stars from being spammed on
        reactions = discord.utils.get(message.reactions, emoji = "⭐")
        if reactions.count != c.star_quota: return # Cannot starboard if wrong amount of reactions
        ...
        if message.content == "": message_content = str(message.attachments[0]) # Prevents error when sending attachments
        else: message_content = message.content
