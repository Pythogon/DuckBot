...
{p}weekschedule: See the shceduled for weekly notifications.""", inline=False) \
            .add_field(name="Counting channel",
                       value=f"This bot provides a counting channel. This channel can be located at "
                             f"<#{c.counting_channel}>. To use the counting channel, you just need to type the number "
                             f"after the last number sent to the channel. Your message will be automatically deleted.",
                       inline=False) \
            .add_field(name="Notifications",
                       value=f"This bot will notify you when it's your turn to take out the trash or to do the "
                             f"dishes. The notification channel is at <#{c.notification_channel}>.") \
            .add_field(name="Starboard",
                       value=f"This bot also provides a starboard for you to use! The starboard channel can be found "
                             f"at <#{c.star_channel}>. To get a message onto the starboard channel, just react with a "
                             f"⭐! If the message gets {c.star_quota} ⭐s, it will be automatically put onto the "
                             f"starboard.",
                       inline=False) \
    ...
    async def metrics(self, ctx, user: discord.User):
        ...
        embed = discord.Embed(title=f"Metrics for {user}", color=c.embed_color) \
            ...
            .add_field(name="Counting",
                       value=f"""Total number of messages in the counting channel: **{userdata["count"]["number"]}**
Number of times failed in the counting channel: **{userdata["count"]["fails"]}**""", inline=False) \
            .add_field(name="Starboard",
                       value=f"Total number of messages added to the starboard: **{userdata['starboard']['number']}**",
                       inline=False) \
